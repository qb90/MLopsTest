...
for token, label in zip(tokenizer.convert_ids_to_tokens(g["input_ids"][0]),g["labels"][0]):
    print(f"{token:_<40} {label}")

tokenized_dataset = conll2003.map(tokenize_and_align_labels, batched=True)

model = AutoModelForTokenClassification.from_pretrained('bert-base-uncased', num_labels=9) # must match
